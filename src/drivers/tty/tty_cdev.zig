...
const TTY_MAJOR: types.major_t = 4;

/// Minor offset for serial ports: ttyS0 = minor 64.
const SERIAL_MINOR_BASE: types.minor_t = 64;
...
pub fn register_serial(name: []const u8) !types.minor_t {
    if (serial_count >= MAX_SERIAL_PORTS) return error.TooManySerialPorts;
    const minor: types.minor_t = SERIAL_MINOR_BASE + @as(types.minor_t, @intCast(serial_count));
